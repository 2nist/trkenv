...
export default function LabMenu({ onOpenModule }: { onOpenModule?: (entry: ModuleEntry) => void }) {
  ...
  if (loading) return <div className="lab-loading">Loading modules...</div>;
  if (error) return <div className="lab-error">Menu error: {error}</div>;

  return (
    <nav className="lab-menu-root">
